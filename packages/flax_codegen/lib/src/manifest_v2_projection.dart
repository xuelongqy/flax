...
/// Immutable Manifest 2 direct-dependency projection for one package root.
///
...
final class FlaxCodegenManifestV2Projection {
  FlaxCodegenManifestV2Projection._({
    required this.source,
    required this.manifest,
    required List<FlaxCodegenManifestV2> packageManifests,
  ...
  final List<FlaxCodegenManifestV2> packageManifests;
  ...
  factory FlaxCodegenManifestV2Projection({
...
final class _OwnerOccurrence {
  ...
  final FlaxCodegenManifestV2Identity identity;
  final FlaxCodegenSourceLocation location;
}

FlaxCodegenManifestV2Projection? _build({
  required FlaxCodegenManifestV2 root,
  required Map<String, FlaxCodegenManifestV2Projection> directDependencies,
  ...
  final byPackage = <String, FlaxCodegenManifestV2>{};
  ...
  for (final package in packages) {
    for (final module in package.modules) {
      final semantic = FlaxCodegenManifestV2Codec.encodeModule(
        module.model.module,
      );
  ...
  return FlaxCodegenManifestV2Projection._(
...
void _diagnoseDirectDependencies({
  ...
  for (final name in actualKeys) {
    if (!expectedSet.contains(name)) {
      diagnostics.add(
        pointer: flaxCodegenManifestV2Pointer('/imports', name),
...
void _addPackage(
  Map<String, FlaxCodegenManifestV2> byPackage,
  Map<String, Set<String>> claimedSources,
  FlaxCodegenManifestV2 package,
...
void _diagnoseBindingNamespaces(
  List<FlaxCodegenManifestV2> packages,
  Map<String, String> packageSources,
  FlaxCodegenManifestV2Diagnostics diagnostics,
) {
  final byNamespace = <String, List<FlaxCodegenManifestV2>>{};
  for (final package in packages) {
    byNamespace
...
void _diagnoseDuplicateModuleIds(
  ...
  final byModuleId = <String, List<(FlaxCodegenManifestV2, int)>>{};
  for (final package in packages) {
  ...
  for (final group in byModuleId.values) {
    ...
    for (final entry in group) {
      ...
      _addForPackage(
        diagnostics,
        packageSources[package.package]!,
        flaxCodegenManifestV2Pointer('/modules', '$index'),
        'Duplicate moduleId.',
      );
    }
  }
...
void _diagnoseDependentOwnership({
  required List<FlaxCodegenManifestV2> packages,
  required Map<String, String> packageSources,
...
Set<String> _importClosure(
  String packageName,
  Map<String, FlaxCodegenManifestV2> byName,
) {
  final seen = <String>{};
...
FlaxCodegenImportedPackage _importedPackage({
  required FlaxCodegenManifestV2 package,
...
String _identityPointer(
  ...
  flaxCodegenManifestV2Pointer(
    flaxCodegenManifestV2Pointer(
      flaxCodegenManifestV2Pointer('/modules', '$moduleIndex'),
      'model',
    ),
    'identities',
  ),
