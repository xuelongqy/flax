...
| Flutter Context, builders, Routes and Widget interfaces | Supported with Flutter-specific semantics | Automatic library binding reuses provider-owned Context types, infers supported BuildContext-to-Widget builder ownership, and attaches supported non-generic Widget interfaces. Route/page/session roles and observers remain explicit; generic proxy inference does not replace Flutter semantics. |
