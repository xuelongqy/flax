...
Status: in progress (coverage map landed; selection waves not started)

## Goal and scope

Grow **explicit** Flax binding coverage after the External Binding Kit is stable.
Follow [ADR 0018](../decisions/0018-binding-coverage-strategy.md): selection YAML lists
types and members; acceptance is selected surface generates and is tested — **not**
full Flutter SDK export coverage or automatic whole-package binding.
...
small explicit shell. First slice: enable `bindings` + `bindingNamespace:
flax.cupertino`; library `package:cupertino_ui/cupertino_ui.dart`;
`CupertinoApp` / `CupertinoPageScaffold` / `CupertinoNavigationBar` /
`CupertinoButton` / theme data. No SDK `package:flutter/cupertino.dart`; no vsync
controllers in that slice.
...
Architecture owns the map and hard-wall calls (this slice). Next implementation:
Project lead sequences Wave A (Core) and Wave E (Integration, worktree-friendly);
UI takes Wave B then Wave D. Codegen supports generate/check; Integration verifies
