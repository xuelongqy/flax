...
registration change. Today's declaration IDs use originating library URIs such as
`package:flutter/src/widgets/framework.dart::Widget`. Those IDs expose private
implementation layout and change when a declaration moves between private source
...
both `sourceIdentity` and `wireId` in Manifest 2. That is the explicit
...
validate imported Manifest 2 shapes against this exact numbering and do not silently
renumber them.
...
v1 does not add aliases or per-symbol override tables.
...
**Per-symbol override tables or v1 aliases.** Rejected. They hide ownership conflicts
and make historical immutability unenforceable. Fail closed instead.
...
This record is the M1 identity contract. It does not implement `bindingNamespace`,
manifest format 2 identity fields, generated pinning, registration uniqueness checks, or
the repository migration. Those remain later milestone work.
