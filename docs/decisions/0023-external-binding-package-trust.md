...
and TypeScript, binding manifests, package tests, public registration symbol, and
package-local adapters under [ADR 0017](0017-package-boundaries.md).
...
public package APIs plus binding manifests. It never reads dependency selection YAML,
tests, or private source.
...
The binding manifest copies `bindingNamespace` and contains the downstream projection
defined in [ADR 0021](0021-external-binding-version-domains.md). It does not copy
selection YAML. Owner-local output paths are not cross-package contracts.
...
the identity and generation projection. Both use shared contract fixtures under
...
Names, license, signing, registry ownership, publication, and support duration remain
...
belongs to Workspace / Tooling. The parsers share a fixture corpus under
`tests/compatibility` without a new published package.
